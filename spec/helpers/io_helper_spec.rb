...
RSpec.describe IOHelper do
  ...
  describe 'print statistic' do
    ...
    it 'values' do
      expect{print_statistic}.to output{include statistic.name, response.difficulty, response.attempts_total, response.attempts_used, response.hints_total, response.hints_used}.to_stdout
    end
  end
  ...
  describe 'print game status' do
    it 'message and values' do
      ...
      expect{console_interface.print_game_status(c_game)}.to output(include I18n.t(:game_status), '15', '2').to_stdout
    end
  end
